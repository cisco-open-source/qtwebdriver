...
void MakeMongooseOptions(const std::string& port,
  ...
  out_options->push_back("extra_mime_types");
  out_options->push_back(".xhtml=application/xhtml+xml");
}
...
//    enable_keep_alive = true;
  ...
  std::string chromedriver_info = base::StringPrintf(
      "ChromeDriver %s", "QtWebKit"/*chrome::kChromeVersion*/);
    ...
    if (cmd_line->HasSwitch("config"))
    {
        ...
        if (file_util::ReadFileToString(configPath, &config_json))
        {
            ...
            if (result_dict->GetString("root", &root))
                cmd_line->AppendSwitchASCII("root", root);
            if (result_dict->GetString("url-base", &url_base))
                cmd_line->AppendSwitchASCII("url-base", url_base);
